...
extern uint8_t led_command;
uint8_t is_blinking_mode = 0;

const char *message_debug_on = "\nD1 triggered, commanded on\n";
const char *message_debug_off = "\nD2 triggered, commanded off\n";
const char *message_debug_blink = "\nD3 triggered, commanded blink\n";
...
void TIM6_IRQHandler(void) {
	...
	// If blinking mode is active, toggle the LED and continue blinking

	if (led_command == 1) {

		led_off();
		UART_send_string(message_debug_off);
		led_command = 0; // Reset to command waiting state

	} else if (led_command == 2) {

		led_on();
		UART_send_string(message_debug_on);
		led_command = 0; // Reset to command waiting state

	} else if (led_command == 3) {
		led_toggle();
		UART_send_string(message_debug_blink);
...
void timer_set_delay_time(uint16_t time) {
        // Only set delay time if not in blinking mode
        TIM6->ARR = time - 1;
}
...
void timer_enable(void) {
    ...
    // Enable timer interrupt, is it necessary??
    // NVIC_EnableIRQ(TIM6_IRQn);
...
void timer_disable(void) {
    ...
    TIM6->CR1 &= ~(1 << 0); // Disable the timer
    // NVIC_DisableIRQ(TIM6_IRQn); // Disable timer interrupt // Is this necessary??
}
