...
void pwm_init(void) {
	...
	TIM3->ARR = 20000-1; // Period must be 20 ms for this RC Servo. 1/20ms = 50 Hz. So we need to obtain 50 Hz result from 1 MHz timer clock. Then we must divide 1 MHz timer clock after prescaler by x to get 50 Hz frequency. Then it's 20.000
